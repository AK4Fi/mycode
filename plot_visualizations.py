...
FAMILY_NAMES = ['Ramnit', 'Lollipop', 'Kelihos_v3', 'Vundo', 'Simda', 'Tracur', 'Kelihos_v1', 'Obfuscator', 'Gamarue']

def load_model(exp_name, modality='all', fusion='cross_attn'):
    model = MultiModalMalwareClassifier(modality=modality, fusion=fusion, attn_dir='cfg2seq').to(DEVICE)
...
def plot_confusion_matrix(model, dataloader):
    ...
    sns.heatmap(cm, annot=True, fmt='d', cmap='Blues', xticklabels=FAMILY_NAMES, yticklabels=FAMILY_NAMES)
...
def plot_tsne(model_single, model_multi, dataloader):
    def get_features(m):
        m.eval()
        feats, labels = [], []
        hook = m.classifier[2].register_forward_hook(lambda module, inp, out: feats.append(out.cpu().numpy()))
        with torch.no_grad():
            for batch in dataloader:
                _ = m(batch.to(DEVICE))
    ...
    sns.scatterplot(x=tsne_s[:, 0], y=tsne_s[:, 1], hue=labels, palette=palette, ax=ax1, legend=False, s=40)
    ax1.set_title("Fig A: Single-Modal (Opcode Only) Feature Space")
    
    sns.scatterplot(x=tsne_m[:, 0], y=tsne_m[:, 1], hue=labels, palette=palette, ax=ax2, s=40)
    ax2.set_title("Fig B: Multi-Modal (Ours) Feature Space")
    ax2.legend(bbox_to_anchor=(1.05, 1), loc='upper left', title="Families", labels=FAMILY_NAMES)
...
def plot_attention_heatmap(model, dataset):
    ...
    cfg_embed = global_mean_pool(x, torch.zeros(sample.x.size(0), dtype=torch.long).to(DEVICE))
    ...
    attn_output, attn_weights = model.cross_attention(
        query=cfg_embed.unsqueeze(1), key=seq_states, value=seq_states, key_padding_mask=(b_mask==0)
    )
    ...
    sns.heatmap([plot_weights], cmap="Reds", xticklabels=plot_tokens, yticklabels=["Attention"])
...
if __name__ == '__main__':
    test_dataset = MalwareMultimodalDataset('./data/test.csv', './data/graphs/', truncate_mode='cfg_guided')
